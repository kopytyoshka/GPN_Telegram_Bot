...
def four_parameters(message):
    ...
    #                                            "супермегаинформацию из мегабазы данных!\n\nЧто вам из инструментов "
    #                                            "подходит больше всего?", reply_markup=buttons)
...
def end_filtration(message, copy_df):
    ...
    # copy_df.to_excel(r"babyFile.xlsx", sheet_name="Ваш результат!", index=False)
    pd.set_option("display.max_column", None)
    pd.set_option("display.max_colwidth", None)
    client.send_message(message.chat.id, "Ваш документ формируется, пожалуйста, подождите!")
    dfi.export(copy_df, 'babyFile.png', table_conversion="selenium", max_rows=-1, max_cols=-1, fontsize=8, dpi=120)
    # result_doc = open("babyFile.png", "rb")
    # client.send_document(message.chat.id, result_doc)
    # result_doc.close()
    image_1 = Image.open(r"babyFile.png")
    im_1 = image_1.convert('RGB')
    im_1.save(r'babyFile.pdf')
    result_doc = open("babyFile.pdf", "rb")
    client.send_document(message.chat.id, result_doc)
    result_doc.close()
    os.remove("babyFile.pdf")
    os.remove("babyFile.png")
    ...
    client.register_next_step_handler(msg, four_parameters)
